...
export default function AgentView() {
  ...
  const getByAgent = (agentId) => {
    ...
    if (fs) {
      res = res.filter(i => i.status === fs);
    }
    
    if (fp) {
      res = res.filter(i => i.priority === fp);
    }
              ...
              return (
                <div key={idx} className="card mb-4">
                  <div className="card-header bg-success text-white">
                    <h5 className="mb-0">Sales Agent: {agent.name} ({leads.length})</h5>
                  </div>
                  <div className="card-body">
                    {leads.length === 0 ? (
                ...
                return (
                  <div className="card mb-4">
                    <div className="card-header bg-warning text-dark">
                      <h5 className="mb-0">Unassigned Leads ({unassigned.length})</h5>
                    </div>
                    <div className="card-body">
                      <div className="list-group">
                        {unassigned.map((x, i) => (
